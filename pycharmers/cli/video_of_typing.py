...
from ..opencv.video_image_handler import VideoWriterCreate, videocodec2ext
from ..utils._colorings import toACCENT, toBLUE, toGREEN, toRED
...
def video_of_typing(argv=sys.argv[1:]):
    ...
    parser = argparse.ArgumentParser(
        prog="video_of_typing", description="Create a typing Video.", add_help=True
    )
    ...
    parser.add_argument(
        "--size",
        action=ListParamProcessorCreate(type=int),
        default=[1080, 1920],
        help="The image size. (W,H)",
    )
    parser.add_argument(
        "--bgRGB",
        action=ListParamProcessorCreate(type=int),
        default=[255, 255, 255],
        help="The color of background image. (RGB)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=5,
        help="The length of the created video. This value is used when 'video' is NOT specified.",
    )
    parser.add_argument(
        "--fps",
        type=float,
        default=None,
        help="The fps of the created video. This value is used when 'video' is NOT specified.",
    )
    parser.add_argument("--image", type=str, default=None, help="The path to input image.")
    parser.add_argument(
        "--margin", type=int, default=0, help="The margin size for pasting video or image."
    )
    parser.add_argument(
        "--align",
        action=ListParamProcessorCreate(type=str),
        default=None,
        help="horizontal and vertical alignment of the content (video/image).",
    )
    parser.add_argument(
        "--out",
        type=str,
    ...
    if W < w:
        warnings.warn(
            f"The output f{toGREEN('width')} is smaller than that of the media, so expand it from {toGREEN(W)} to {toGREEN(w)}."
        )
        W = w
    if H < h:
        warnings.warn(
            f"The output f{toGREEN('height')} is smaller than that of the media, so expand it from {toGREEN(H)} to {toGREEN(h)}."
        )
        H = h
    is_ok, out_video, out_path = VideoWriterCreate(
        out_path=out_path, codec=codec, fps=fps, size=(W, H), verbose=True
    )
    if is_ok:
        ...
        type_writer = TypeWriter(
            total_frame_count=n, typing_json_paths=args.typing, verbose=verbose
        )
        ...
        out_video.release()
        if verbose:
            print(f"Typing Video is saved at {toBLUE(out_path)}")
...
class BaseTypeWriter:
    def __init__(
        self, total_frame_count: int, typing_json_paths: Tuple[str] = (), verbose: bool = True
    ):
        ...
        for path in typing_json_paths:
            drawing_func, messages = self.typing_data_create(
                json_path=path, total_frame_count=total_frame_count
            )
            self.print(*messages)
            self.drawing_functions.append(drawing_func)

    @staticmethod
    def typing_data_create(
        json_path: str,
        total_frame_count: int,
        fontsize: int = 30,
        textRGB: Tuple[int, int, int] = (0, 0, 0),
        **kwargs,
    ) -> Tuple[Callable[[Image.Image, int], Image.Image], List[str]]:
            ...
            if s <= curt_frame_count:
                if last or curt_frame_count <= e:
                    img, _ = draw_text_in_pil(
                        text=typing_texts[
                            max(min(int((curt_frame_count - s) // span), num_typing_texts - 1), 0)
                        ],
                        img=img,
        ...
        def draw_typing_text(img, curt_frame_count: int):
            if s <= curt_frame_count:
                if last or curt_frame_count <= e:
                    x, y = (X, Y)
                    text = typing_texts[
                        : max(min(int((curt_frame_count - s) // span), num_typing_texts), 0)
                    ]
                    for code, cls in split_code(text):
                        img, (x, y) = draw_text_in_pil(
                            ...
                            ttfontname=ttfontname,
                            fontsize=fontsize,
                            textRGB=cls2bgr.get(cls, textBGR),
                            ret_position="word",
                            **typing_data,
                        )
                        x += Xspan
